...
describe("emitFunctionGraphBody", () => {
  it("passes a single parameter straight through to Return", () => {
...
describe("logic.function mode: blueprint — validation", () => {
  it("rejects more than one Return node", () => {
    const flow = makeFlow([
      {
        id: "fn1",
        type: "logic.function",
        ...
        data: {
          ...
          params: "n",
          mode: "blueprint",
          graph: { nodes: [graphReturn("ret1"), graphReturn("ret2")], edges: [] },
        },
      },
    ]);
    const result = validateFlow(flow);
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.message.includes("at most one Return node"))).toBe(true);
  });
