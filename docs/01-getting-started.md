...
2. Hit **Compile** to generate an Express server from the flow. For a multi-file project,
...
3. Hit **Run Server** to spawn the generated server as a real child process and see its
   logs streamed live in the editor. **Stop Server** kills it.
4. Or skip the built-in runner entirely — the generated files are plain `.js`, so you can
   `cd` into the project directory yourself and run `node server.js` (after `npm install`
   in that directory, if the flow declares any npm dependencies).
