...
written in a [Handler Function](/node-reference/logic/handler-function)
...
Function](/node-reference/logic/handler-function)'s
blueprint-mode body — a Route can no longer wire to Send JSON directly, only to a
Handler Function.

<NodePreview type="handler.sendJson" height={200} width={280} />
