...
`new Map(...)`/`new Set(...)`/`new WeakSet(...)`, an **error**-typed variable gets wrapped
in `new Error(...)` — see [Error nodes](/node-reference/error#the-error-variable-type) —
and so on) — a *wired* value is never reformatted, since it's already a proper expression.
